...
def pack(function: t.Callable[..., _T2],
         /) -> t.Callable[[t.Iterable[_T1]], _T2]:
...
def reverse_contour_coordinates(contour: _ContourT, /) -> _ContourT:
    reversed_vertices = [reverse_point_coordinates(vertex)
                         for vertex in contour.vertices]
    result = type(contour)(reversed_vertices)
    if result.orientation is not contour.orientation:
        result = type(contour)([*reversed_vertices[:1],
                                *reversed_vertices[:0:-1]])
    return result


def reverse_multipolygon_polygons(multipolygon: _MultipolygonT,
                                  /) -> _MultipolygonT:
    return type(multipolygon)(reverse_sequence(multipolygon.polygons))


def reverse_multipolygon_coordinates(multipolygon: _MultipolygonT,
                                     /) -> _MultipolygonT:
...
def reverse_multisegment_coordinates(multisegment: _MultisegmentT,
                                     /) -> _MultisegmentT:
...
def to_pairs(values: _st.SearchStrategy[_T],
             /) -> _st.SearchStrategy[t.Tuple[_T, _T]]:
    return _st.tuples(values, values)


def to_triplets(values: _st.SearchStrategy[_T],
                /) -> _st.SearchStrategy[t.Tuple[_T, _T, _T]]:
