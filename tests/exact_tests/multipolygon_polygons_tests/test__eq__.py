...
def test_transitivity(first: t.Sequence[Polygon],
                      second: t.Sequence[Polygon],
                      third: t.Sequence[Polygon]) -> None:
    assert implication(first == second and second == third, first == third)


@given(strategies.multipolygons_polygons, strategies.multipolygons_polygons)
def test_alternatives(first: t.Sequence[Polygon],
                      second: t.Sequence[Polygon]) -> None:
    assert equivalence(first == second, not first != second)


@given(strategies.multipolygons_polygons, strategies.multipolygons_polygons)
def test_reversals(first: t.Sequence[Polygon],
                   second: t.Sequence[Polygon]) -> None:
