...
impl Edge {
    pub(super) fn is_under<Point: PartialEq + Orient>(
        ...
        let other_right_orientation =
            self.orientation_of(&endpoints[other.right_point_index], endpoints);
        if other_left_orientation == other_right_orientation {
            ...
            let left_orientation =
                other.orientation_of(&endpoints[self.left_point_index], endpoints);
            let right_orientation =
                other.orientation_of(&endpoints[self.right_point_index], endpoints);
