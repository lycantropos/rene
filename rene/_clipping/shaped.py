...
class Operation(ABC, t.Generic[hints.Scalar]):
    ...
    def reduce_events(
        ...
        are_internal: t.List[bool] = []
        connectivity = self._events_to_connectivity(events)
        contours_ids = [UNDEFINED_INDEX] * len(events)
        contours_vertices: t.List[t.List[hints.Point[hints.Scalar]]] = []
        depths: t.List[int] = []
        holes: t.List[t.List[int]] = []
        parents: t.List[int] = []
        ...
        result: t.List[hints.Polygon[hints.Scalar]] = []
    ...
    def _is_common_polyline_component_left_event(
            self, event: Event, /
    ) -> bool:
    ...
    def _process_contour_events(self,
                                ...
                                contour_id: int,
                                are_events_processed: t.List[bool],
                                are_from_in_to_out: t.List[bool],
                                contours_ids: t.List[int],
                                events_ids: t.Sequence[int],
                                /) -> None:
    ...
    def _to_contour_events(
            ...
            visited_endpoints_positions: t.List[int],
            /
    ) -> t.List[Event]:
        assert is_left_event(event)
        result = [event]
