...
def intersect_multipolygon_with_multipolygon(
    ...
    first_common_area_polygons = [
        first_polygons[polygon_id] for polygon_id in first_common_area_polygons_ids
    ]
    second_common_area_polygons = [
        second_polygons[polygon_id] for polygon_id in second_common_area_polygons_ids
    ]
    max_min_x = max(
        min(
            first_boxes[polygon_id].min_x
            for polygon_id in first_common_area_polygons_ids
        ),
        min(
            second_boxes[polygon_id].min_x
    ...
    operation = ShapedIntersection.from_segments_iterables(
        (
            ...
            if (
                max_min_x <= max(segment.start.x, segment.end.x)
...
def intersect_multipolygon_with_multisegmental(
    ...
    first_common_continuum_polygons = [
        first_polygons[polygon_id] for polygon_id in first_common_continuum_polygons_ids
    ]
...
def intersect_multipolygon_with_segment(
    ...
    if not first_common_continuum_polygons_ids:
        return empty_cls()
    first_common_continuum_polygons = [
        first_polygons[segment_id] for segment_id in first_common_continuum_polygons_ids
    ]
    max_min_x = max(
        min(
            first_boxes[polygon_id].min_x
...
def intersect_multisegmental_with_multipolygon(
    ...
    first_common_continuum_segments = [
        first_segments[segment_id] for segment_id in first_common_continuum_segments_ids
    ]
    ...
    return collect_maybe_empty_segments(
        [
            ...
            for maybe_segment in [
                ...
                if do_boxes_have_common_continuum(
                    first_segment.bounding_box, second_bounding_box
                )
            ]
...
def intersect_segments_with_common_continuum_bounding_boxes(
    ...
    return (
        segment_cls(max(start, other_start), min(end, other_end))
        if (
            (
                start == other_start
                or (orienteer(end, start, other_start) is Orientation.COLLINEAR)
            )
            and (
                end == other_end
                or (orienteer(end, start, other_end) is Orientation.COLLINEAR)
            )
        )
        else None
    )
    ...
    return collect_maybe_empty_segments(
        [
            ...
            for maybe_segment in [
                ...
                if do_boxes_have_common_continuum(
                    second_segment.bounding_box, first_bounding_box
                )
            ]
            if maybe_segment is not None
        ],
...
def intersect_segment_with_segment(
    ...
    if do_boxes_have_no_common_continuum(first.bounding_box, second.bounding_box):
        return empty_cls()
