...
class Trapezoidation(t.Generic[hints.Scalar]):
    ...
    def _from_box_with_edges(cls,
        ...
        _add_edge_to_single_trapezoid(
                0, _box_to_trapezoid(box, edges, nodes), edges, nodes
        )
...
def _add_edge_to_first_trapezoid(
        ...
        nodes: t.List[Node[hints.Scalar]],
        /
) -> t.Tuple[Trapezoid[hints.Scalar], Trapezoid[hints.Scalar]]:
    edge = edges[edge_index]
    above, below = (
        _create_trapezoid(edge.left_point, trapezoid.right_point,
                          edge_index,
    ...
    if edge.left_point == trapezoid.left_point:
        upper_left = _maybe_get_trapezoid(trapezoid.upper_left_leaf_index,
                                          nodes)
        above.set_as_upper_left(upper_left)
        lower_left = _maybe_get_trapezoid(trapezoid.lower_left_leaf_index,
                                          nodes)
        below.set_as_lower_left(lower_left)
    else:
        left = _create_trapezoid(trapezoid.left_point, edge.left_point,
                                 trapezoid.below_edge_index,
                                 trapezoid.above_edge_index, nodes)
        left.set_as_lower_left(
                _maybe_get_trapezoid(trapezoid.lower_left_leaf_index, nodes)
        )
        left.set_as_upper_left(
                _maybe_get_trapezoid(trapezoid.upper_left_leaf_index, nodes)
        )
    ...
    above.set_as_upper_right(
            _maybe_get_trapezoid(trapezoid.upper_right_leaf_index, nodes)
    )
    below.set_as_lower_right(
            _maybe_get_trapezoid(trapezoid.lower_right_leaf_index, nodes)
    )
...
def _add_edge_to_last_trapezoid(
        edge_index: int,
        trapezoid: Trapezoid[hints.Scalar],
        prev_above: Trapezoid[hints.Scalar],
        prev_below: Trapezoid[hints.Scalar],
        edges: t.Sequence[Edge[hints.Scalar]],
        nodes: t.List[Node[hints.Scalar]],
        /
) -> None:
    ...
    if prev_above.above_edge_index is trapezoid.above_edge_index:
        ...
        above = _create_trapezoid(trapezoid.left_point, edge.right_point,
    ...
    if prev_below.below_edge_index is trapezoid.below_edge_index:
        ...
        below = _create_trapezoid(trapezoid.left_point, edge.right_point,
    ...
    if edge.right_point == trapezoid.right_point:
        above.set_as_upper_right(
                _maybe_get_trapezoid(trapezoid.upper_right_leaf_index,
                                     nodes)
        )
        below.set_as_lower_right(
                _maybe_get_trapezoid(trapezoid.lower_right_leaf_index,
                                     nodes)
        )
    else:
        right = _create_trapezoid(edge.right_point, trapezoid.right_point,
                                  trapezoid.below_edge_index,
                                  trapezoid.above_edge_index, nodes)
        ...
        replacement_node_index = _create_x_node(edge.right_point,
...
def _add_edge_to_middle_trapezoid(
    ...
    if prev_above.above_edge_index is trapezoid.above_edge_index:
        ...
        above = _create_trapezoid(trapezoid.left_point,
    ...
    if prev_below.below_edge_index is trapezoid.below_edge_index:
        ...
        below = _create_trapezoid(trapezoid.left_point,
                                  trapezoid.right_point,
                                  trapezoid.below_edge_index, edge_index,
                                  nodes)
        below.set_as_upper_left(prev_below)
...
def _add_edge_to_single_trapezoid(edge_index: int,
    ...
    above, below = (
        _create_trapezoid(edge.left_point, edge.right_point, edge_index,
                          trapezoid.above_edge_index, nodes),
        _create_trapezoid(edge.left_point, edge.right_point,
                          trapezoid.below_edge_index, edge_index, nodes)
    )
    replacement_node_index = _create_y_node(edge_index, below.leaf_index,
                                            above.leaf_index, nodes)
...
def _box_to_trapezoid(box: hints.Box[hints.Scalar],
    ...
    return _create_trapezoid(point_cls(min_x, min_y),
                             point_cls(max_x, min_y),
                             below_edge_index, above_edge_index, nodes)


def _create_trapezoid(left_point: hints.Point[hints.Scalar],
...
def _create_x_node(point: hints.Point[hints.Scalar],
                   left_node_index: int,
...
def _find_intersecting_trapezoids(edge_index: int,
                                  edges: t.Sequence[Edge[hints.Scalar]],
                                  nodes: t.Sequence[Node[hints.Scalar]],
                                  /) -> t.List[Trapezoid[hints.Scalar]]:
    edge = edges[edge_index]
    cursor = nodes[0]
    for _ in repeat(None, len(nodes) - 1):
        candidate = cursor.search_edge_node(edge, edges, nodes)
...
def _maybe_get_trapezoid(
        index: t.Optional[int], nodes: t.Sequence[Node[hints.Scalar]], /
) -> t.Optional[Trapezoid[hints.Scalar]]:
    if index is None:
        return None
    node = nodes[index]
    assert isinstance(node, Leaf), node
