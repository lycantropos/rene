...
class Operation(Generic[hints.Scalar]):
    ...
    def classify_event(self, event: Event, /) -> EventKind:
        left_event_position = left_event_to_position(self._to_left_event(event))
    ...
    def has_edges_cross(self, same_start_events: Iterable[Event], /) -> bool:
        ...
        for event in same_start_events:
            ...
            if self._overlap_kinds[left_event_position] is OverlapKind.NONE:
                ...
                event_is_inside = self._other_have_interior_to_left[left_event_position]
...
class RelationState(Generic[hints.Scalar]):
    def update(
        ...
        if all_same(
            operation.is_event_from_first_operand(event) for event in same_start_events
        ):
