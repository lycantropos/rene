...
from rene._utils import all_same, is_even, square, to_sorted_pair
...
class Operation(t.Generic[hints.Scalar]):
    ...
    def has_crossing(self, same_start_events: t.Sequence[Event]) -> bool:
        ...
        from_first_operand_events_count = sum(
            self.is_event_from_first_operand(event) for event in same_start_events
        )
        if not (1 < from_first_operand_events_count < len(same_start_events) - 1):
            # for crossing angles there should be
            # at least two pairs of segments from each operand
            return False
        from_first_events: list[Event] = []
        from_second_events: list[Event] = []
        for event in same_start_events:
            (
                from_first_events
                if self.is_event_from_first_operand(event)
                else from_second_events
            ).append(event)
    ...
    def to_signed_point_event_squared_cosine(
        ...
        return (
            square(dot_product) if dot_product > 0 else -square(dot_product)
...
def has_two_elements(iterator: t.Iterator[t.Any]) -> bool:
    return next(iterator, None) is not None and next(iterator, None) is not None
    ...
    return (
        ...
        if first_half_orientation is Orientation.COLLINEAR
            ...
            if second_half_orientation is Orientation.COLLINEAR
                ...
                (first_half_orientation is second_half_orientation)
...
class RelationState(t.Generic[hints.Scalar]):
    def update(
        ...
        if operation.has_intersection(same_start_events):
            ...
            assert all(
                not operation.is_event_from_first_operand(event)
                for event in same_start_events
            )
    ...
    def _detect_touch_or_overlap(
        ...
        for _, group in chain(
            groupby(
                filter(is_event_left, same_start_events),
